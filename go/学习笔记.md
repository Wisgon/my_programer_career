...
48. goroutine中，不能改变全局变量，要用channel才行，所以切记如果要在goroutine中使用全局变量，则要用channel传进去；<br><br>

49. go不能直接  xx, err := yy() 这样的方式赋值xx这个全局变量：
...
50. go使用new和var的分别是，new返回的是指针，而var则是声明了一个变量：
