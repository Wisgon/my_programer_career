...
44. 当select和for联合使用时，用break是跳不出循环的，要用break label来跳出循环：
        ...
        for {
            ...
            case <-time.After(time.Second * time.Duration(2)):
                ...
                if i == 5{
                    ...
                    break ForEnd  // 如果这里只是break，则只是跳出select
                }
                fmt.Println("inside the select: ")
            }
            // 如果break安排在这里，则for循环只有一次就退出了
...
45. 要在方法中改变结构体的值时，注意要方法的接受者必须是指针接受者：
    ...
    func (ps PointStruct) Change() {
    	a := "fdsf"
    	ps.aaa = a         // 无法改变ps.aaa的值，只能在这里临时改变，函数体外还是nil值
    }
    
    func (ps *PointStruct) PointChange() {
    	a := "fdsf"
    	ps.aaa = a
    }
    ...
    func main() {
    	ps := PointStruct{}
    	ps.Change()
    	fmt.Println(ps.aaa)  // 为空，因为Change方法是值接收者，不能改变ps
        ps.PointChange()
        fmt.Println(ps.aaa)  // "fdsf"
    }
    ```

    <br><br>
    
46. go在import自定义包的事后，import那里必须是文件夹的名字，而引用的时候，就是package包名，如：
    a.go在demo/simple_test/文件夹下，内容为：
    ...
    在main.go中，调用MyFunc()时，代码如下：

    ```go
    package main
    
    import "demo/simple_test" // 这里必须是文件夹名字simple_test
    
    func main() {
        simpletest.MyFunc() // 这里调用的时候，要用package的名字
    }
    ```

    <br><br>
    
47. go交叉编译命令：`$CGO_ENABLE=0 GOOS=windows GOARCH=amd64 go build -o idcard_face_alarm.exe idcard_face_alarm.go`<br><br>
